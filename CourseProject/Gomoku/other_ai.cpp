...
// hash表相关
const int _HASH_EXACT = 0;
const int _HASH_ALPHA = 1;
const int _HASH_BETA = 2;
...
int AI::SearchHash(int depth, int alpha, int beta)
{
	...
	if (p_hash->key == zobristKey)
	{
		...
		{
			if (p_hash->hashtype == _HASH_EXACT)
			{
				return p_hash->value;
			}
			else if (p_hash->hashtype == _HASH_ALPHA && p_hash->value <= alpha)
			{
				return p_hash->value;
			}
			else if (p_hash->hashtype == _HASH_BETA && p_hash->value >= beta)
			{
				return p_hash->value;
			}
		}
...
int AI::AlphaBeta(int depth, int alpha, int beta, MoveLine* pline)
{
	...
	while (pos.row != -1)
	{
		...
		if (value >= beta)
		{
			UpdateHash(depth, value, _HASH_BETA);
			RecordPVMove(pos);
			return value;
		}
		if (value > best.value)
