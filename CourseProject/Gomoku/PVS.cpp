...
# File Function: 实现基于PVS算法的五子棋ai（最终版本）
# Author       : YunPeng Xu(徐云鹏)
# Nickname     : 奕衍盯阵，鉴定为奕？悟！
# Update Date  : 2024/11/8
**********************************************/
...
const int kMaxDepth        = 20;            // 最大搜索深度
const int kMaxMoves        = 15;            // 每层最多候选落子数
const int kTimeLimit       = 1400;          // 限制时间(ms)
/*棋盘类型*/   
...
const int T_COMBINE        = 6;				// 组合（活三+活三，活三+冲四）
const int T_BLOCK_FOUR     = 5;				// 冲四
...
const double g_value[9]    = {S_ONE, S_BLOCK_TWO, S_LIVE_TWO, S_BLOCK_THREE, S_LIVE_THREE, S_BLOCK_FOUR, S_COMBINE, S_LIVE_FOUR, S_FIVE_LINE};
const double ATK_RATIO     = 1;             // 进攻系数
const double DEF_RATIO     = 1.2;           // 防守系数
/*方向*/
...
double GetBoardScore(int player_color)
{
	...
	int player_block_four_num = 0;			  	     // 上次循环我方的冲四数量，用于判断活四，连续冲四
    int enemy_block_four_num = 0;			  	     // 上次循环敌方的冲四数量，用于判断活四，连续冲四
    int player_live_three_num = 0;			  	     // 上次循环我方的活三数量
    int enemy_live_three_num = 0;			  	     // 上次循环敌方的活三数量
...
*/
double AlphaBeta(int depth, double alpha, double beta, int player_color)
{
    //如果当前局面已经搜索过了
    ...
    //如果深度为0 或 棋盘满了
    if(depth == 0 || g_chess_count == kBoardSize * kBoardSize)
    {
...
void GetBestMove(int player_color, Position &best_move)
{
    ...
    else if(g_chess_count == 5)//执白
    {
        ...
        {
            UpdateBZ(6 + kCompensate, 7 + kCompensate, WHITE, -1);
            best_move = Position{6 + kCompensate, 7 + kCompensate};
            return;
        }
        else if(g_board[4 + kCompensate][4 + kCompensate].type == BLACK)
        {
            UpdateBZ(5 + kCompensate, 4 + kCompensate, WHITE, -1);
            best_move = Position{5 + kCompensate, 4 + kCompensate};
            return;
        }
        //如果对面活三，八卦阵起
    ...
    //如果双方都没有五连点，开始搜索
    if(!g_is_aggressive)//白棋ab搜索总是输，遂贪心，狠狠滴贪！
    {
        double max_score = -10 * S_FIVE_LINE;
        for(i = kBoardBegin; i <= kBoardEnd; ++i)
        {
            for(j = kBoardBegin; j <= kBoardEnd; ++j)
            {
                if(g_board[i][j].type != EMPTY || g_board[i][j].num_in_2 == 0)
                    continue;
                double score = 0;
                score += 1 * GetNodeScore(i, j, player_color);
                score += 5 * GetNodeScore(i, j, enemy_color);
                if(score > max_score)
                {
                    max_score = score;
                    best_move = Position{i, j};
                }
                else if(score == max_score && g_board[i][j].num_in_2 > g_board[best_move.row][best_move.col].num_in_2)
                    best_move = Position{i, j};
            }
        }
    ...
    GetOptionalMove(optional_moves, player_color);
    //迭代加深搜索
    for(int iterative_depth = 3; iterative_depth <= 7; iterative_depth += 1)
    {        
        ...
        for(auto &move : optional_moves)
        {
            if(g_is_timeout || std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - g_start_time).count() > kTimeLimit)
                break;
            UpdateBZ(move.row, move.col, player_color, -1);
            score = -AlphaBeta(iterative_depth - 1, -beta, -alpha, enemy_color);
            UpdateBZ(move.row, move.col, EMPTY, player_color);
            if(g_is_timeout || std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - g_start_time).count() > kTimeLimit)
                break;
            if(score > max_score)
        ...
        fflush(stdout);
#endif
        if(std::abs(max_score - last_score) < 1e-6 && best_move == last_best_move && iterative_depth >= 6)
        {
            break;
        }
    }
